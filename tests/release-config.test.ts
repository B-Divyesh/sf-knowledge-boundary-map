...
describe('static hosting response policy', () => {
  ...
  it('@finding:404-metadata gives the static 404 complete route metadata and shared controls', async () => {
    ...
    for (const token of ['name="description"', 'rel="canonical"', 'property="og:title"', 'name="twitter:card"', 'id="theme-toggle"', 'Source on GitHub', 'build polish-3']) expect(html).toContain(token);
  });
});
