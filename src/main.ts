...
function mapPage(): string {
  ...
  return layout(`<main class="site-main" id="main">
    ...
    <ul class="boundary-ledger" aria-label="Self-assessed boundary summary">${counts.map(([status, count]) => `<li class="ledger-item ${status}"><strong>${count}</strong><span>${statusLabel(status)}</span></li>`).join('')}</ul>
...
function upgradePage(): string {
  ...
  </section><p class="microcopy" style="margin-top:24px">Sociobot/Dodo is the merchant of record. Refunds are handled there and revoke the license. Read our <a href="/terms" data-route>terms</a> and <a href="/privacy" data-route>privacy notice</a>.</p></main>`);
}
